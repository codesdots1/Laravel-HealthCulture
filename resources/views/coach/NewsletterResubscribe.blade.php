                    ...
                    <div class="p-4 mb-md-5 mb-4 newslettersubscribe-content semibold">
                        ...
                        <div class="NewsletterUnsubscribe_content ps-3 pe-3">
                                <p class="text-md-center text-center " >
                                    You have been <b class="heavy">successfully unsubscribed </b> from the CoachCulture email newsletter.
                                </p>
                                <p class="text-md-center text-center">
                                   Miss us already? The feeling is mutual.
                                </p> 
                            <p class="text-md-center text-center" >
                                Enter your email address below to <b class="heavy">resubscribe</b>:
                            </p>
                            <form id="NewsletterResubscribe"   class="screen5-form needs-validation mt-2" method="post" role="form" data-toggle="validator" novalidate>
                                {{ csrf_field() }}
...
$("#NewsletterResubscribe").validate({
    rules: {
        field: {
            required: true,
            email: true
    ...
    submitHandler: function(form) {
         $.ajax({
            type: "POST",
            url: "{{route('saveNewsletterResubscribe')}}",
            data:     $("#NewsletterResubscribe").serializeArray(),
            success: function(response) {
                var response = JSON.parse(response);
                   ...
                   if (response.error) {
                       ...
                       $('#NewsletterResubscribe').trigger('reset');
